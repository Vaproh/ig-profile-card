...
if [ "$PROXY_ENABLED" = "true" ] && [ -n "$PROXY_SERVER" ]; then
    ...
    CAMOFOX_CMD="cd $CAMOFOX_DIR && PROXY_STRATEGY=backconnect PROXY_BACKCONNECT_HOST=$PROXY_HOST PROXY_BACKCONNECT_PORT=${PROXY_PORT:-823} PROXY_USERNAME=$PROXY_USERNAME PROXY_PASSWORD=$PROXY_PASSWORD CAMOFOX_PORT=9377 npm start"
else
